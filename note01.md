...
每个html元素都能抽象成一个类似盒子样的模型，包括内容区，内边距，边框，外边距

![](./note01/01.png)

IE盒子模型的宽高包含了内容区，内边距，边框；而标准盒模型的宽高只包含内容区
...
说白了就是让标签有含义，别动不动就套div，结构看着也舒服，便于维护，更重要的是可以让搜索引擎更好的理解。大公司好像都很注意这点，看来以后要改改习惯了。
...
位于 head 元素内部，用于存储一些页面的相关信息，可以告诉浏览器如何显示内容，存一些keywords方便SEO等等

### 响应式布局

通过设置meta标签中的viewport和css的媒体查询可以实现响应式的布局
...
## 面试题补充
### doctype的作用？
告诉浏览器的解析器，用什么文档的类型规范来解析这个文档。doctype不存在的话默认会以兼容模式加载页面。
### 前端如何优化seo？
- 语义化标签
