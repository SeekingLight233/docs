...
js 中的每一个变量或者说函数都会有一个作用的范围，这个范围就叫作用域。
而作用域链指的是在 js 中查找一个变量的过程，会从最内层开始找，逐步找到最外层。
...
::: warning
闭包中自由变量的查找，是在函数定义的地方往上找，而不是在函数执行的地方往上找。
:::

## this 的指向判断问题

`this`取什么样的值，实在函数执行的时候被确认的，而不是在定义的时候被确认。
